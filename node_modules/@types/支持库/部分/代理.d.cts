...
接口 代理处理程序类<T 扩展 实例> {
    ...
    //@@{ 属性键类:PropertyKey }
    //@@{ 取自身属性描述符:getOwnPropertyDescriptor, 目标:target }
    取自身属性描述符?(目标: T, p: 属性键类): 属性描述符类 | 未定;
    //@@{ 属性键类:PropertyKey }
    //@@{ 存在:has, 目标:target }
    存在?(目标: T, p: 属性键类): 真假;
    //@@{ 属性键类:PropertyKey } 
    //@@{ 获取:get, 目标:target, 接收器:receiver }
    获取?(目标: T, p: 属性键类, 接收器: 任意): 任意;
    //@@{ 属性键类:PropertyKey }
    //@@{ 设置:set, 目标:target, 值:value, 接收器:receiver }
    设置?(目标: T, p: 属性键类, 值: 任意, 接收器: 任意): 真假;
    //@@{ 属性键类:PropertyKey }
    //@@{ 删除属性:deleteProperty, 目标:target }
    删除属性?(目标: T, p: 属性键类): 真假;
    //@@{ 属性描述符类:PropertyDescriptor }
    //@@{ 属性键类:PropertyKey } 
    //@@{ 定义属性:defineProperty, 目标:target, 特性集:attributes }
    定义属性?(目标: T, p: 属性键类, 特性集: 属性描述符类): 真假;
    //@@{ 属性键类:PropertyKey } 
    enumerate?(target: T): 属性键类[];
    //@@{ 属性键类:PropertyKey }
    //@@{ 自身键集:ownKeys, 目标:target }
    自身键集?(目标: T): 属性键类[];
...
//@@{ 代理构造函数类:ProxyConstructor, 可撤销的:revocable, 目标:target, 处理程序:handler, 代理:proxy, 撤销:revoke, 目标:target, 处理程序:handler }
接口 代理构造函数类 {
